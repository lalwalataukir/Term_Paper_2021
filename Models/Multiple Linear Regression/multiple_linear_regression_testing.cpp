...
int main()
{
    ...
    vector<long double> coefficients = {0.052794, -0.667856, 0.016929, 0.281758, 0.309720, -0.061102, 0.175801, 0.327907, -0.020731};
    ...
    for (int i = 0 ; i < N ; i++)
    {
        ...
        value = value + coefficients[1] * radius_mean[i];
        value = value + coefficients[2] * texture_mean[i];
        value = value + coefficients[3] * perimeter_mean[i];
        value = value + coefficients[4] * area_mean[i];
        value = value + coefficients[5] * smoothness_mean[i];
        value = value + coefficients[6] * compactness_mean[i];
        value = value + coefficients[7] * concave_points_mean[i];
        value = value + coefficients[8] * fractal_dimension_mean[i];
